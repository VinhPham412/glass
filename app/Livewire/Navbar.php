<?php
	...
	class Navbar extends Component {
		...
		public function mount() {
			...
			$this->customer = Customer::where('email', $this->email_user)
				->first();
			if ($this->customer) {
				$this->list_order = Order::where('customer_id', $this->customer->id)
					->get();
			} else {
				$this->list_order = [];
			}
		...
		public function updatingSearchInput() {
			if (empty($this->searchInput)) {
				$this->list_product_search = [];
			} else {
				$this->list_product_search = Product::where('name', 'like', '%' . $this->searchInput . '%')
					->get();
			}
		}
		...
		public function dat_hang() {
			...
			foreach ($this->cart as $version_id => $quantity) {
				...
				if ($version && $quantity > $version->stock) {
					...
					$cart_updated = true;
					Notification::make()
						->title("Số lượng sản phẩm '{$version->product->name}' vượt quá tồn kho!")
						->body("Đã giảm số lượng xuống {$version->stock}.")
						->warning()
						->send();
				}
			}
			...
			if (empty($this->name_user) || empty($this->email_user) || empty($this->phone_user) || empty($this->address_user)) {
				Notification::make()
					->title('Vui lòng điền đầy đủ thông tin mua của bạn!')
					->warning()
					->send();
				return;
			}
			...
			$customer_order = null;
			if (Customer::where('email', $this->email_user)
				->first()) {
				$customer = Customer::where('email', $this->email_user)
					->first();
